...
public class UpdateTeacher extends AppCompatActivity {
    ...
    private DatabaseReference reference,bdRef;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateTeacherImage.setOnClickListener(view -> {
            openGallery();
        });
    ...
    private void deleteData() {
        String uniqueKey = reference.getKey();
        assert uniqueKey != null;
        reference.child(uniqueKey).removeValue()
    ...
    private void UpdateData(String s) {
        ...
        if (uniqueKey != null) {
            reference.child(category).child(uniqueKey).updateChildren(hp).addOnSuccessListener(new OnSuccessListener() {
                ...
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(UpdateTeacher.this, "Something Wrong", Toast.LENGTH_SHORT).show();
                }
            });
        }
    ...
    private void uploadImage() {
        ...
        boolean compress = bitmap.compress(Bitmap.CompressFormat.JPEG, 50, baos);
